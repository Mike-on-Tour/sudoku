...
class mot_sudoku_main
{
	...
	public function __construct(\phpbb\auth\auth $auth, \phpbb\config\config $config, \phpbb\db\driver\driver_interface $db, \phpbb\controller\helper $helper,
								...
								\phpbb\template\template $template, \phpbb\user $user, $root_path, $mot_sudoku_classic_table, $mot_sudoku_games_table, $mot_sudoku_stats_table)
	{
		$this->auth = $auth;
		$this->config = $config;
		$this->db = $db;
...
//					'MOT_SUDOKU_PLAYER_LINE'		=> $player_line,
		...
		$mean_points = $games_solved ? $total_points / $games_solved : 0;
		...
		$this->template->assign_vars([
			...
			'MOT_SUDOKU_MEAN_POINTS'		=> $mean_points,
			...
			'MOT_SUDOKU_NEGATIVE_POINTS'	=> -1 * (($game_reset * $this->config['mot_sudoku_reset_cost']) + ($game_buy_digit * $this->config['mot_sudoku_number_cost']) + ($game_helper * $this->config['mot_sudoku_helper_cost']) + ($game_level * $this->config['mot_sudoku_level_cost'])),
		]);
		...
		if ($this->request->is_ajax())
		{
			...
			else
			{
				...
				else
				{
					...
					{
						case 'c':
							$sql_arr['player_line'] = json_encode($this->classic_array);
							break;
					...
					$result = [
						...
						'reset'				=> $sql_arr['reset'],
						'points'			=> $sql_arr['points'],
						'negative_points'	=> -1 * (($sql_arr['reset'] * $this->config['mot_sudoku_reset_cost']) + ($sql_arr['buy_digit'] * $this->config['mot_sudoku_number_cost']) + ($sql_arr['helper'] * $this->config['mot_sudoku_helper_cost']) + ($sql_arr['level'] * $this->config['mot_sudoku_level_cost'])),
					];
				}
			}
